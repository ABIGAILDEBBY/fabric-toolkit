...
class TestPushParts:
    ...
    @pytest.mark.integration
    def test_202_polling_timeout_returns_false(self):
        push_resp = MagicMock()
        push_resp.status_code = 202
        push_resp.headers = {'Location': 'https://api.example.com/ops/1'}

        still_running = MagicMock()
        still_running.status_code = 202
        still_running.json.return_value = {'status': 'Running'}

        with patch('requests.post', return_value=push_resp), \
             patch('requests.get', return_value=still_running), \
             patch('time.sleep'):
            result = dis.push_parts('item-timeout', [])
...
class TestDisableFabricScheduleAdditional:
    ...
    @pytest.mark.integration
    def test_dates_updated_to_2099_2100_when_schedule_disabled(self):
        original = {
            'schedules': [{
                'enabled': True,
                'configuration': {
        ...
        parts = [{'path': '.schedules', 'payload': schedule_payload}]
        ...
        with patch('requests.post') as mock_post:
            mock_post.side_effect = [get_resp, push_resp]
            ok, msg = dis.disable_fabric_schedule('item-dates')

        assert ok is True
        push_body = mock_post.call_args_list[1].kwargs.get('json') \
            or mock_post.call_args_list[1][1]['json']
        schedule_part = next(
            p for p in push_body['definition']['parts'] if p['path'] == '.schedules'
        )
        pushed_data = json.loads(base64.b64decode(schedule_part['payload']).decode())
        cfg = pushed_data['schedules'][0]['configuration']
        assert pushed_data['schedules'][0]['enabled'] is False
        assert cfg['startDateTime'] == '2099-01-01T00:00:00'
        assert cfg['endDateTime'] == '2100-12-31T23:59:59'
